...
function exactHints(text){const s=String(text||''),patterns=[/\b([A-Z0-9]{2,6})\s*[#-]\s*([A-Za-z0-9★]+)\b/i,/\[([A-Z0-9]{2,6})\]\s*#?\s*([A-Za-z0-9★]+)/i,/\bset\s*[:=-]?\s*([A-Z0-9]{2,6}).{0,25}?\b(?:collector|number|no\.?|#)\s*[:=-]?\s*([A-Za-z0-9★]+)/i];for(const p of patterns){const m=s.match(p);if(m)return{set:m[1].toLowerCase(),collector_number:m[2]}}return{}}
...
function shopifyRows(source,products){const rows=[];for(const p of products){if(!relevant(p))continue;const baseText=[p.title,p.product_type,p.vendor,Array.isArray(p.tags)?p.tags.join(' '):p.tags].filter(Boolean).join(' '),isSealed=sealedWords.test(baseText);for(const v of p.variants||[]){if(v.available===false)continue;const price=Number(v.price);if(!(price>0))continue;const text=[p.title,v.title,v.sku,baseText].filter(Boolean).join(' '),h=exactHints(text),finish=finishFrom(text),condition=conditionFrom(text),compare=Number(v.compare_at_price)||null;if(isSealed){rows.push({candidate_only:true,type:'sealed',name:p.title,product_name:p.title,store:source.name,url:productUrl(source,p.handle),price,compare_at_price:compare,detail:[v.title,compare?`store compare-at $${compare.toFixed(2)}`:''].filter(Boolean).join(' • '),source_id:source.id});continue}if(!h.set||!h.collector_number)continue;rows.push({type:'single',name:cleanCardName(p.title),card_name:cleanCardName(p.title),store:source.name,url:productUrl(source,p.handle),price,set:h.set,collector_number:h.collector_number,finish,condition,detail:[h.set.toUpperCase(),`#${h.collector_number}`,finish,condition].filter(Boolean).join(' • '),source_id:source.id})}}return rows}
async function fetchJson(url,timeout=6500){const ctl=new AbortController(),t=setTimeout(()=>ctl.abort(),timeout);try{const r=await fetch(url,{signal:ctl.signal,headers:{'user-agent':'MTGDealHunter/5.5 market-source-library','accept':'application/json'}});if(!r.ok)throw Error(`HTTP ${r.status}`);return await r.json()}finally{clearTimeout(t)}}
