...
function median(a){return quantile([...a].sort((x,y)=>x-y),.5)}
...
function verdict(score,s,historyPoints,consistency,baselineQuality,regimeShift){
  ...
  if(consistency<50||baselineQuality<50){confidence=Math.min(confidence,54);if(action==='BUY')action='WATCH'}
...
export default async()=>{
  try{
    ...
    const ranked=candidates.map((card,index)=>{
      ...
      const rawHistValue=Math.max(0,-histVs30)*2+Math.max(0,-histVs90)*1+Math.max(0,-belowQ25)*1+(percentile90!==null&&percentile90<=20?6:percentile90!==null&&percentile90<=35?3:0);
      ...
      const signal={value:+valueScore.toFixed(1),momentum:+momentumScore.toFixed(1),demand:+demandScore.toFixed(1),entry:entryScore,premium:premiumScore,penalties,historical_points:hp.length,hist_vs_30:+histVs30.toFixed(2),hist_vs_90:+histVs90.toFixed(2),percentile_90:percentile90,hist_trend:+histTrend.toFixed(2),historical_current:histCurrent,median_30:med30,median_90:med90,source_gap_pct:sourceGap==null?null:+sourceGap.toFixed(2),pricing_consistency:consistency,baseline_quality:baselineQuality,regime_shift:regimeShift,regime_delta:+regimeDelta.toFixed(2),outlier_share:+outlierShare.toFixed(3)};
      const recommendation=verdict(score,signal,hp.length,consistency,baselineQuality,regimeShift);
      history[card.id]=[...points,{t:now,price:livePrice}].slice(-HISTORY_POINTS);
      return {id:card.id,name:card.name,set:card.set,set_name:card.set_name,collector_number:card.collector_number,price:livePrice,score,rank_source:index+1,drop24,drop72,move1,vs6,premium,hist_vs_30:histVs30,hist_vs_90:histVs90,percentile_90:percentile90,historical_points:hp.length,historical_current:histCurrent,median_30:med30,median_90:med90,pricing_consistency:consistency,baseline_quality:baselineQuality,regime_shift:regimeShift,regime_delta:regimeDelta,source_gap_pct:sourceGap,action:recommendation.action,confidence:recommendation.confidence,reason:reasons.join(' • '),image:card.image_uris?.normal||card.card_faces?.[0]?.image_uris?.normal||card.image_uris?.small||card.card_faces?.[0]?.image_uris?.small||null,scryfall_uri:card.scryfall_uri,signal};
