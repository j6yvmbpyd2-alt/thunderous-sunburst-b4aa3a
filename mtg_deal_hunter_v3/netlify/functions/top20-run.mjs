...
function verdict(score,signals){
  const evidence=[signals.value>=12,signals.momentum>=3,signals.demand>=8,signals.premium>=8,signals.penalties===0].filter(Boolean).length;
  let action='PASS';
  if(score>=48&&signals.value>=10&&signals.penalties<12) action='BUY';
  else if(score>=28) action='WATCH';
  const confidence=clamp(Math.round(35+evidence*10+Math.min(25,signals.value/2)+Math.min(10,signals.momentum)-Math.min(20,signals.penalties)),25,95);
...
export default async () => {
  try{
    ...
    const r=await fetch(url,{headers:{"user-agent":"MTGDealHunter/3.8","accept":"application/json"}});
    ...
    const ranked=candidates.map((card,index)=>{
      ...
      const penalties=supplyRisk+genericPenalty+chasePenalty;
      ...
      const signal={value:Number(valueScore.toFixed(1)),momentum:Number(momentumScore.toFixed(1)),demand:Number(demandScore.toFixed(1)),entry:entryScore,premium:premiumScore,penalties};
      const recommendation=verdict(score,signal);
    ...
    const payload={ok:true,model:'opportunity-v3',updated_at:new Date().toISOString(),top20,history,candidate_count:candidates.length};
