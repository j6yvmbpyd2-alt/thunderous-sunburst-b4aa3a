(()=>{
  ...
  async function render(){
    ...
    box.innerHTML='<div class="tiny good">Enhanced Watchlist v2 loaded</div><div class="notice" style="margin-top:8px">Loading exact printing images and intelligence summaries…</div>';
    const cards=await Promise.all(rows.map(x=>exact(x.id)));
    box.innerHTML='<div class="tiny good">Enhanced Watchlist v2 loaded</div>'+rows.map((x,i)=>{const c=cards[i],img=c?.image_uris?.normal||c?.card_faces?.[0]?.image_uris?.normal||'',p=pick(c,x.finish)||Number(x.last)||0,target=Number(x.target)||0,hit=target&&p&&p<=target;return`<div class="item" data-watch-index="${i}"><div style="display:grid;grid-template-columns:${img?'96px 1fr':'1fr'};gap:12px;align-items:start">${img?`<img src="${esc(img)}" style="width:96px;border-radius:10px;display:block">`:''}<div><div class="row"><div><b>${esc(x.name)}</b><div class="muted">${esc(x.set)} • #${esc(x.num)} • ${esc(x.finish||'nonfoil')}</div></div>${p?`<b>${money(p)}</b>`:''}</div><div style="margin-top:6px"><span class="pill ${hit?'good':''}">Target ${target?money(target):'—'}</span>${hit?'<span class="pill good">TARGET HIT</span>':''}</div><div id="watchIntel${i}" class="notice" style="margin-top:8px">Loading Purchase Decision…</div><div class="buttons"><button class="secondary small watch-decision" data-id="${esc(x.id)}" data-finish="${esc(x.finish||'any')}">Full Purchase Decision</button><button class="ghost small watch-remove" data-index="${i}">Remove</button></div></div></div></div>`}).join('');
    box.querySelectorAll('.watch-decision').forEach(b=>b.onclick=()=>window.openPurchaseDecision?.(b.dataset.id,b.dataset.finish||'any'));
