(()=>{
  ...
  function svgChart(points){
    const vals=points.map(x=>Number(x.price)).filter(x=>x>0); if(vals.length<2)return '<div class="empty">More price snapshots are needed for a chart.</div>';
  ...
  async function openIntel(id){
    ...
    try{
      ...
      const histReady=Number(i.historical_points||0)>0;
      card.innerHTML=`<div class="row"><div><h2>${esc(x.name)}</h2><div class="muted">${esc(x.set_name||x.set)} • #${esc(x.collector_number||'')}</div></div><button id="intelClose" class="secondary" style="width:auto;margin:0">Close</button></div><div style="display:grid;grid-template-columns:${x.image?'130px 1fr':'1fr'};gap:14px;margin-top:12px">${x.image?`<img src="${esc(x.image)}" style="width:130px;border-radius:10px">`:''}<div><div class="price">${money(i.current)}</div><span class="pill ${cls}"><b>${esc(i.decision)}</b></span><span class="pill">Buy Now ${i.buy_confidence}%</span><span class="pill">${esc(i.trend)}</span><div class="grid2" style="margin-top:10px"><div class="kpi"><span class="muted">Fair buy</span><b>${money(i.fair_buy)}</b></div><div class="kpi"><span class="muted">Strong buy</span><b>${money(i.strong_buy)}</b></div></div><div class="muted" style="margin-top:9px">Current price percentile: ${i.percentile==null?'—':i.percentile+'%'}</div><div style="margin-top:7px"><span class="pill">Historical ${Number(i.historical_confidence||0)}%</span><span class="pill">Live ${Number(i.live_confidence||0)}%</span></div><div class="tiny" style="margin-top:5px">${histReady?`${i.historical_points} daily historical points + ${i.live_points} live hourly points`:`Historical cache not seeded yet • ${i.live_points} live hourly points`}</div></div></div><div style="margin-top:14px"><h2>Price history</h2>${svgChart(h)}</div><div class="grid3" style="margin-top:12px"><div class="kpi"><span class="muted">30d avg</span><b>${i.avg30?money(i.avg30):'—'}</b></div><div class="kpi"><span class="muted">30d low</span><b>${i.low30?money(i.low30):'—'}</b></div><div class="kpi"><span class="muted">30d high</span><b>${i.high30?money(i.high30):'—'}</b></div></div><div class="grid3" style="margin-top:8px"><div class="kpi"><span class="muted">90d avg</span><b>${i.avg90?money(i.avg90):'—'}</b></div><div class="kpi"><span class="muted">90d low</span><b>${i.low90?money(i.low90):'—'}</b></div><div class="kpi"><span class="muted">90d high</span><b>${i.high90?money(i.high90):'—'}</b></div></div><div class="grid3" style="margin-top:8px"><div class="kpi"><span class="muted">24h avg</span><b>${i.avg24?money(i.avg24):'—'}</b></div><div class="kpi"><span class="muted">24h low</span><b>${i.low24?money(i.low24):'—'}</b></div><div class="kpi"><span class="muted">24h high</span><b>${i.high24?money(i.high24):'—'}</b></div></div><div class="notice" style="margin-top:12px">${esc(x.reason||'')}</div>${i.history_source?`<div class="tiny" style="margin-top:8px">Historical source: ${esc(i.history_source.source||'MTGJSON')} • ${esc(i.history_source.provider||'TCGplayer')} • ${esc(i.history_source.resolution||'daily')}</div>`:''}${x.scryfall_uri?`<a href="${esc(x.scryfall_uri)}" style="display:block;margin-top:10px;color:#93c5fd">View exact printing</a>`:''}`;
